...
#$ -l h_rt=02:00:00
#$ -j y
#$ -o output/$JOB_ID_pretrain_deit_tiny_pyto_fdb21k_renderportiontossd.out
#$ -N pret_vit_tiny_p16_224_pyto_fdb21k_portion
#$ -l USE_BEEOND=1
cat $JOB_SCRIPT
echo "..................................................................................................."
echo "JOB ID: ---- >>>>>>   $JOB_ID"
...
mpirun --bind-to socket -machinefile $SGE_JOB_HOSTLIST --use-hwthread-cpus -npernode 50 -np 1750 python mpi_ifs_search_egl.py --ngpus-pernode 4 --category 21000 --save_dir /beeond
...
# For CPU
mpirun --bind-to none -machinefile $SGE_JOB_HOSTLIST -npernode 1 -np 35 mkdir ${SSD}/fdb21k_cpu
...
# For CPU
mpirun --bind-to socket -machinefile $SGE_JOB_HOSTLIST --use-hwthread-cpus -npernode 50 -np 1750 python mpi_cpu.py --image_res 362 --save_root ${SSD}/fdb21k_cpu --load_root /beeond/csv_rate0.2_category21000_points200000


##### Debug local
readlink -f ${SSD}
ls ${SSD}
mpirun --bind-to none -machinefile $SGE_JOB_HOSTLIST -npernode 1 -np 35 ls ${SSD}/fdb21k_cpu 
mpirun --bind-to none -machinefile $SGE_JOB_HOSTLIST -npernode 1 -np 35 find ${SSD}/fdb21k_cpu -type f -print |wc -l
...
export EXPERIMENT=localShuf_35n
...
python portiontossd_pretrain.py ${DATASET} \
    ...
    --epochs ${EPOCHS} --opt adamw --lr ${LR} --weight-decay 0.05 --deit-scale 8960.0 \
