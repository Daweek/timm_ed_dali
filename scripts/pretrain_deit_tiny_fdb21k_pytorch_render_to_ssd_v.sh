...
############# Render to local SSD
export LOCALDIR=/local/${JOB_ID}.1.gpu
...
mpirun --display-allocation --bind-to socket -machinefile $SGE_JOB_HOSTLIST --use-hwthread-cpus -npernode 80 -np 8000 python mpi_ifs_search_egl.py --ngpus-pernode 4 --category 21000 --save_dir ${LOCALDIR}/ --local_ranks

echo "Start rendering to local ..."
# mpirun --bind-to none --use-hwthread-cpus -np 80 python mpi_cpu.py --save_root ${LOCALDIR}/fdb1k_cpu
mpirun --display-allocation --bind-to socket -machinefile $SGE_JOB_HOSTLIST --use-hwthread-cpus -npernode 80 -np 8000 python mpi_gpu.py --ngpus-pernode 4 --image_res 362 --save_root ${LOCALDIR}/fdb21k --load_root ${LOCALDIR}/csv_rate0.2_category21000_points200000 --local_ranks
# du -sh ${DATASET}
cd ../../
##################################
...
export EXPERIMENT=speed_localrank_100n
