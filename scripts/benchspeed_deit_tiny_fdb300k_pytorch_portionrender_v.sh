...
#$ -l h_rt=20:00:00
#$ -j y
#$ -o output/$JOB_ID_pretrain_deit_tiny_pyto_fdb300k_renderportiontossd.out
#$ -N pret_vit_tiny_p16_224_pyto_fdb300k_portion
#$ -l USE_BEEOND=1
...
export DATASET=${LOCALDIR}/fdb300k_${RENDER_HWD}
...
mpirun --bind-to socket -machinefile $SGE_JOB_HOSTLIST --use-hwthread-cpus -npernode 50 -np 5000 python mpi_ifs_search_egl.py --ngpus-pernode 4 --category 300000 --save_dir /beeond
...
# For CPU
mpirun --bind-to none -machinefile $SGE_JOB_HOSTLIST -npernode 1 -np 100 mkdir ${SSD}/fdb300k_cpu
...
# For CPU
mpirun --bind-to socket -machinefile $SGE_JOB_HOSTLIST --use-hwthread-cpus -npernode 50 -np 5000 python mpi_cpu.py --image_res 362 --save_root ${SSD}/fdb300k_cpu --load_root /beeond/csv_rate0.2_category300000_points200000


##### Debug local
readlink -f ${SSD}
ls ${SSD}
mpirun --bind-to none -machinefile $SGE_JOB_HOSTLIST -npernode 1 -np 100 ls ${SSD}/fdb300k_cpu 
mpirun --bind-to none -machinefile $SGE_JOB_HOSTLIST -npernode 1 -np 100 find ${SSD}/fdb300k_cpu -type f -print |wc -l
