...
#$ -l h_rt=05:00:00
#$ -j y
#$ -o output/$JOB_ID_pretrain_deit_tiny_pyto_fdb50k_renderportiontossd.out
#$ -N pret_vit_tiny_p16_224_pyto_fdb50k_portion
#$ -l USE_BEEOND=1
cat $JOB_SCRIPT
echo "..................................................................................................."
echo "JOB ID: ---- >>>>>>   $JOB_ID"
...
mpirun --bind-to socket -machinefile $SGE_JOB_HOSTLIST --use-hwthread-cpus -npernode 50 -np 5000 python mpi_ifs_search_egl.py --ngpus-pernode 4 --category 50000 --save_dir /beeond
...
# For CPU
mpirun --bind-to none -machinefile $SGE_JOB_HOSTLIST -npernode 1 -np 100 mkdir ${SSD}/fdb50k_cpu
...
# For CPU
mpirun --bind-to socket -machinefile $SGE_JOB_HOSTLIST --use-hwthread-cpus -npernode 50 -np 5000 python mpi_cpu.py --image_res 362 --save_root ${SSD}/fdb50k_cpu --load_root /beeond/csv_rate0.2_category50000_points200000


##### Debug local
readlink -f ${SSD}
ls ${SSD}
mpirun --bind-to none -machinefile $SGE_JOB_HOSTLIST -npernode 1 -np 100 ls ${SSD}/fdb50k_cpu 
mpirun --bind-to none -machinefile $SGE_JOB_HOSTLIST -npernode 1 -np 100 find ${SSD}/fdb50k_cpu -type f -print |wc -l
